...
"For pin configuration details, see [GPIO configuration](#)."
...
"Configure the device tree overlay as described in [Device tree configuration](#). For details about individual properties, see the [Device tree bindings reference](#)."
...
"Before starting this tutorial, complete the steps in [Installing the nRF Connect SDK](#)."

!!! note

    Consider reusing content in these scenarios.
    For example, use includes in Sphinx where appropriate.

### Place cross-references strategically

**At the beginning of a topic** for prerequisites:

"This guide assumes you have completed [Setting up your development environment](#)."

**Within body text** for related concepts:

"The [device tree](#) defines the hardware configuration for your application."
...
"The `bt_le_adv_start()` function is documented in the [Bluetooth advertising API](#)."

**Referencing hardware documentation:**

"For the complete pin assignment, see the [nRF52840 DK hardware description](#)."
