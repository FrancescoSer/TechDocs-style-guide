...
For example: REQUEST_AND_END action
...
For example: The STOPPED event occurs when...
...
For example: makefile, HEX file, header file, C file, PDF file
...
For example: a write, a read, erase operation
...
For example: In the **Item list**, click **Desktop**.
...
For example: Before RADIO can transmit a packet, it must first ramp-up in TX mode.
For example: Before the RADIO peripheral can transmit a packet, it must first ramp-up in TX mode.
...
For example: Connect the `GND` pin to pin `P0.05`.
...
For example: *Bluetooth Core Specification*
...
For example: COUNTERTOP register
...
For example: PINx.DETECT signal
...
For example: TASKS_CAPTURE[n] TASKS_CLEAR

When referring to the group or category, the name can be shortened to the category name.

For example:  If one or more of the CAPTURE tasks and the CLEAR task are triggered at the same time, that is, within the same period of PCLK16M, the CAPTURE tasks are prioritized.
...
For example: The 128-bit characteristic UUID is `00001630-1212-EFDE-1524-785FEABCD123`.
...
* Symbols can vary by region (For example:, # is "hash" in UK, "pound" in US)
